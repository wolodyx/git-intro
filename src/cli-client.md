...
Большинство команд выполняют действие по введенным заранее аргументам, выводят информацию на экран и завершают выполнение как и подобает интерфейсу командной строки (`git status`, `git add .`, `git log -1`).
Некоторые команды в определенном состоянии переходят в интерактивный режим, используя:
* `vim`, `nano` или другой текстовый редактор для ввода или редактирования многострочного текста (`git commit`);
* `less` для просмотра текстовых данных, не помещающихся на экране (`git log --all --graph`);
* ввод с клавиатуры для интерактивной выборки (`git add --patch`).
...
1. Перечислите не менее 5 команд Git и объясните коротко, что они делают.
...
1. Можно ли одновременно работать с хранилищем из разных Git-клиентов?
  Почему?
