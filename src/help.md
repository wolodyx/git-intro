...
```{note}
"Отобразить справочную страницу" -- значит открыть html-страницу со справкой в веб-браузере (в OS Windows) или открыть в просмотрщике текстовых файлов `less` (в ОС Ubuntu).
```
...
1. Какую из команд Git используют для работы со справочной системой?
